...
const AdminSignup = () => {
  ...
  const handleSubmit = async value => {
    setIsLoading(true)
    try {
      const userAdmin = await createUserWithEmailAndPassword(
        auth,
        value.email,
        value.password
      )

      const uid = userAdmin.user.uid

      if (!uid) return toast.error("L'ID utilisateur est introuvable.")

      const userdbRef = doc(db, 'admin', uid)
      await setDoc(userdbRef, {
        name: value.name,
        email: value.email,
        create_at: serverTimestamp() || new Date(),
        role: 'admin',
      })
      setLoggedIn(true)
    } catch (error) {
